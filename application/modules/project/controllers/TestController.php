...
class Project_TestController extends Custom_Controller_Action_Application_Project_Abstract
{
    ...
    public function addOtherTestGroupAction()
    {
        $this->_checkAccess(Application_Model_RoleAction::TEST_ADD, true);

        $this->_setTranslateTitle();
        $this->view->form = $this->_getAddOtherTestGroupForm();
    }
    ...
    public function addOtherTestGroupProcessAction()
    {
        $this->_checkAccess(Application_Model_RoleAction::TEST_ADD, true);
        ...
        if (!$request->isPost()) {
            return $this->projectRedirect(array(), 'test_add_other_test_group');
        }

        $form = $this->_getAddOtherTestGroupForm();
        $post = $form->preparePostData($request->getPost());
        ...
        if (!$form->isValid($post)) {
            ...
            return $this->render('add-other-test-group');
        }
        ...
        if ($testMapper->addOtherTestGroup($test)) {
            $this->_messageBox->set($t->translate('statusSuccess'), Custom_MessageBox::TYPE_INFO);
        } else {
            $this->_messageBox->set($t->translate('statusError'), Custom_MessageBox::TYPE_ERROR);
        }
    ...
    private function _getAddOtherTestGroupForm()
    {
        $form = new Project_Form_AddOtherTestGroup(array(
            'action' => $this->_projectUrl(array(), 'test_add_other_test_group_process'),
            'method' => 'post',
            'projectId' => $this->_project->getId()
        ));
